...
def main():
    ...
    with col1:
        ...
        st.text_area("請貼上問題或程式碼：", height=200, key="question_content")
    ...
    with col2:
        ...
        st.text_area("助教可直接在此修改 AI 草稿：", height=400, key="draft_content")
        st.markdown("---")
        st.write("請對解答品質進行評分：")
        
        b1, b2, _ = st.columns([1,1,4])
        if b1.button(f"{SATISFIED_EMOJI} 滿意"):
            save_solution_to_db(st.session_state.last_question, st.session_state.draft_content, SATISFIED_EMOJI, "資訊類科", st.session_state.teacher_name)
            st.toast("已送出滿意評價並完成紀錄", icon="🎉")
        ...
        if b2.button(f"{UNSATISFIED_EMOJI} 不滿意"):
            save_solution_to_db(st.session_state.last_question, st.session_state.draft_content, UNSATISFIED_EMOJI, "資訊類科", st.session_state.teacher_name)
            st.toast("已紀錄不滿意回饋", icon="⚠️")
            for k in ["draft_content", "question_content", "last_question", "uploaded_files"]: st.session_state.pop(k, None)
            st.rerun()
